...
#prints all polynomials nubered using a list with polynomial varaibles from a varables list
def display_poly_list_numbered(poly_list, varaibles):
    for i in range(len(poly_list)):
        text = "\n (" + str(i+1) + ") " + poly_to_text(poly_list[i], varaibles)
        print(text)

#prints all polynomials in a list of lists of polynomials numbering both list levels with polynomial varaibles from a varables list
def display_poly_list_list_numbered(poly_list_list, varaibles):
    for i in range(len(poly_list_list)):
        for j in range(len(poly_list_list[i])):
            if j == 0:
                ...
                temp = "  "
            text = "\n " + temp + " (" + str(j+1) + ") " + poly_to_text(poly_list_list[i][j], varaibles)
            print(text)
